...
num_rbf_neurons = min(100, len(X_train) // 2)  
...
sigma = compute_sigma(X_train, centers)
...
X_rbf = rbf_kernel(X_train, centers, sigma)




rfr_model = RandomForestRegressor(n_estimators=500, max_depth=4, random_state=42)
rfr_model.fit(X_rbf, y_train)


y_pred_train = rfr_model.predict(X_rbf)
y_pred_test = rfr_model.predict(X_rbf)
...
predicted_temp = rfr_model.predict(user_input_rbf)[0]
